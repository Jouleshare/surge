...
def run_smoke_tests(_mock_call_loadout):
    ...
    jobs_api = client.get("/jobs?format=json", headers={"Authorization": "Basic YWRtaW46am91bGUyMDI2"})
    assert jobs_api.status_code == 200
    assert jobs_api.get_json()["count"] >= 1

    artifacts_api = client.get("/artifacts?format=json", headers={"Authorization": "Basic YWRtaW46am91bGUyMDI2"})
